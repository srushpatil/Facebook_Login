...
export default function Facebook() {
  ...
  const [formData, setFormData] = useState({ country_code: "", phone: "", user_name: "" });
  ...
  useEffect(() => {
    axios.get("https://api.silocloud.io/api/v1/public/countries").then((response) => {
      const countryData = response.data.data.countries.map((country) => ({
        name: country.name, code: `+${country.phonecode}`
      }));
  ...
  return (
    <div className="container mt-5">
      <FacebookLogin appId="1937254353445888" autoLoad={true} fields="name,email" callback={responseFacebook} icon="fa-facebook" cssClass="fb-btn" />
        ...
        <Modal.Body className="popup-form-body">
          <Form className="popup-content">
            <Form.Group className="mb-3">
              ...
              <Form.Control type="text" placeholder="Enter country code" name="country_code"
                value={formData.country_code} onChange={handleInputChange} onKeyDown={handleKeyDown}
                className="ip-details" autoComplete="off" />
              {filteredCountries.length > 0 && (
                <div className="suggestions-wrapper">
                  <ul className="suggestions-list">
                    {filteredCountries.map((country, index) => (
                      <li key={index} ref={(el) => (suggestionRefs.current[index] = el)}
                          onClick={() => handleCountrySelect(country.code)} className={index === activeIndex ? "active" : ""}>
            ...
            <Form.Group className="mb-3">
              ...
              <Form.Control type="text" placeholder="Enter user name" name="user_name"
                value={formData.user_name} onChange={handleInputChange} className="ip-details" />
            ...
            <Form.Group className="mb-3">
              ...
              <Form.Control type="tel" placeholder="Enter phone number" name="phone"
                value={formData.phone} onChange={handleInputChange} className="ip-details" />
